      ...
      db.all("SELECT * FROM `jwt`", async (error, results, fields) => {
        ...
        jwt.verify(results[0].token, process.env.JWT_SECRET, async (err) => {
          if (err) {
            return res
              .status(401)
              .send({ status: "error", message: "Token หมดอายุ!" });
          }
          try {
            ...
            db.all(
              ...
              async (error, results, fields) => {
                ...
                upload(req, res, async (err) => {
                  ...
                  try {
                    // Step 5: Process QR code (if using Jimp and jsQr, uncomment and implement)
                    // const jimp = await Jimp.read(req.file.buffer);
                    // const optimalWidth = 500;
                    // const optimalHeight = 500;
                    // const resizedJimp = jimp.resize(optimalWidth, optimalHeight, Jimp.RESIZE_BILINEAR);
                    // const qr = jsQr(resizedJimp.bitmap.data, resizedJimp.bitmap.width, resizedJimp.bitmap.height);

                    // Step 6: Handle QR code data or use req.body.QR directly
                    const { QR } = req.body;

                    // Step 7: Prepare and send API request
                    const config = {
                      method: "POST",
                    ...
                    console.error(error);
                    if (!error.response) {
                      res.send({ status: "error", message: "Invalid slip!" });
                    } else if (
                      ...
                      // Step 8: Handle token expiration or invalid token
                      // Assuming login and token update functionality exists
                      login(DEVICE, VERSION).then((login) => {
                        db.all(
                          ...
                          async (error, results, fields) => {
                            ...
                            if (results.length === 0) {
                              db.run(
                                "INSERT INTO `access_token` (`data`) VALUES (?)",
                                [login.access_token],
                                async (error, results, fields) => {
                                  if (error) {
                                    console.error(
                                      "Error inserting access token:",
                                      error
                                    );
                                    return;
                                  }
                                  res.send({
                                    message: `Created access token successfully! ${login.access_token}`,
                                  });
                                }
                              );
                            } else {
                              db.run(
                                "UPDATE `access_token` SET `data` = ?",
                                [login.access_token],
                                async (error, results, fields) => {
                                  if (error) {
                                    res.send({
                                  ...
                                  res.send({
                                    message: `Updated access token successfully! ${login.access_token}`,
                                  });
                                }
                              );
                            }
    ...
    try {
      db.all("SELECT * FROM `access_token`", async (error, results) => {
        ...
        try {
          ...
          // console.log(error)
          console.error("Error processing QR code:", error.response.data);
          if (!error.response) {
